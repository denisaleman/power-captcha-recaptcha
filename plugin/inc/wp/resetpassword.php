...
function pwrcap_resetpassword_form_add_render() {
	...
	/**
	 * Fires after CAPTCHA render function is hooked to the WordPress reset password form.
	 *
	 * This action is triggered when the plugin conditionally attaches a CAPTCHA render
	 * function to the WordPress reset password form via the `resetpass_form` action hook.
	 *
	 * The hook is only added if:
	 * - CAPTCHA for the reset password form is enabled via plugin settings.
	 ...
	 * @param string $hook         The action hook where CAPTCHA was attached (typically 'resetpass_form').
	 */
	do_action( 'pwrcap_resetpassword_form_add_render', $captcha_type, $hook );
}
